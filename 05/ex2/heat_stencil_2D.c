...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        // implement heat porpagation using openmp collapse and the finite difference method Laplace operator
        #pragma omp parallel for collapse(2) shared(N,A,B,source_x,source_y)
        for (int i = 0; i < N; i++){
            for(int j = 0; j < N; j++){
                ...
                double temp_left = (j != 0) ? A[IND(i,j-1)] : temp;
                double temp_right = (j != N-1) ? A[IND(i,j+1)] : temp;
                double temp_up = (i != 0) ? A[IND(i-1,j)] : temp;
                double temp_down = (i != N-1) ? A[IND(i+1,j)] : temp;

                // calculate new temperature at current position using the continuous Laplace operator
                B[IND(i,j)] = temp + 0.2 * (temp_left + temp_right + temp_up + temp_down + (-4 * temp));
            }
        }
